/*
Datawarehouse cleardown
=======================

Overview: Deletes all data from all tables in a database, respecting FKs. Very useful 
during development of a data warehouse in my experience!

Instructions: Change the :setvar statement to include the name of the DB that you want to 
empty and execute in SQLCMD (or in SSMS in SQLCMD mode

Jamie Thomson
2009-09-08
*/

:setvar db YourDBNameHere
...
BEGIN TRY
	...
	BEGIN
			...
			SET @vSQL = 'DELETE FROM $(db).' + @schemaTableName;
			EXEC (@vSQL);
			PRINT 'Deleted ' + CAST(@@ROWCOUNT AS VARCHAR) + ' rows from $(db).' + @schemaTableName;
			FETCH NEXT FROM tables_cursor INTO @schemaName, @tableName;
	END
END TRY
BEGIN CATCH
